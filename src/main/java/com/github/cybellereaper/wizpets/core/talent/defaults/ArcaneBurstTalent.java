...
public final class ArcaneBurstTalent implements PetTalent {
    ...
    public void onAttack(ActivePet pet, LivingEntity target, double baseDamage) {
        ...
        target.getWorld().spawnParticle(Particle.END_ROD, target.getLocation().add(0.0, 1.0, 0.0), 15, 0.2, 0.4, 0.2, 0.0);
        pet.getOwner().sendActionBar(Component.text("§5Arcane Burst deals " + String.format(Locale.US, "%.1f", bonus) + " bonus damage!"));
        cooldown = 60;
    }
}
