...
public record PetRecord(
  ...
  public PetRecord withBreedCount(int count) {
    return new PetRecord(
        displayName, evs, ivs, talentIds, generation, count, mountUnlocked, flightUnlocked);
  }

  public PetRecord withMountUnlocked(boolean unlocked) {
    return new PetRecord(
        displayName, evs, ivs, talentIds, generation, breedCount, unlocked, flightUnlocked);
  }

  public PetRecord withFlightUnlocked(boolean unlocked) {
    return new PetRecord(
        displayName, evs, ivs, talentIds, generation, breedCount, mountUnlocked, unlocked);
  }

  public PetRecord withEvs(StatSet newEvs) {
    return new PetRecord(
        displayName,
        Objects.requireNonNull(newEvs, "newEvs"),
        ivs,
        talentIds,
        generation,
        breedCount,
  ...
  public PetRecord withIvs(StatSet newIvs) {
    return new PetRecord(
        displayName,
        evs,
        Objects.requireNonNull(newIvs, "newIvs"),
        talentIds,
        generation,
        breedCount,
        mountUnlocked,
  ...
  public PetRecord withGeneration(int newGeneration) {
    return new PetRecord(
        displayName, evs, ivs, talentIds, newGeneration, breedCount, mountUnlocked, flightUnlocked);
  }
}
